defmodule OnlineShop.AccountsTest do
  ...
  describe "confirm_user_by_token/1" do
    ...
    test "confirms the user, returns the updated struct, and deletes the token", %{
      user: user,
      token: token
    } do
      assert {:ok, confirmed_user} = Accounts.confirm_user_by_token(token)
      assert confirmed_user.confirmed_at
      assert confirmed_user.confirmed_at == Repo.get!(User, user.id).confirmed_at
      refute Repo.get_by(UserToken, user_id: user.id, context: "confirm")
    end

    test "returns error for invalid token" do
      assert :error = Accounts.confirm_user_by_token("oops")
    end

    test "returns error for expired token", %{token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])

      assert :error = Accounts.confirm_user_by_token(token)
    end
  end
